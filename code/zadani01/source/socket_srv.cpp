...
void handle_client(int l_sock_client)
{
    ...
    {
        exit_with_error(l_sock_client, "Client sent 'close' request to close connection.");
    }
    ...
    log_msg(LOG_INFO, "[level: %d, x: %d, y: %d]", level, x, y);
    ...
    log_msg(LOG_INFO, "image path: %s", image_path);
...
void execute_convert(const char *image_path, uint16_t x, uint16_t y)
{
    if (x > 0 && y > 0)
    {
        ...
        execlp("convert", "convert", "-resample", resolution, image_path, "-", (char *)NULL);
    }
    else
    {
        execlp("convert", "convert", image_path, "-", (char *)NULL);
    }
}