...
int main(int t_narg, char **t_args)
{
	...
	while (1)
	{
		int seconds = rand() % 5 + 3;
		log_msg(LOG_INFO, "Waiting %d seconds.", seconds);
		sleep(seconds);

		int random_bit = rand() % 2;
		if (random_bit)
		{
			log_msg(LOG_INFO, "Sending UP...");
			const char *msg = "UP\n";
			write(l_sock_server, msg, strlen(msg));
		}
		else
		{
			log_msg(LOG_INFO, "Sending DOWN...");
			const char *msg = "DOWN\n";
			write(l_sock_server, msg, strlen(msg));
		}

		int bytes_read = read(l_sock_server, buffer, sizeof(buffer));
		if (bytes_read < 0)
		{
		...
		{
			buffer[bytes_read] = '\0';
			log_msg(LOG_INFO, "Received: %s", buffer);
		}
	}
